...
Файл `.env` уже создан с настройками по умолчанию для локальной разработки.
...
**Переменные окружения:**
- `VITE_APP_MODE` - режим работы: `dev` (по умолчанию) или `live`
  - В режиме `dev` запросы проксируются на `http://localhost:3000/api`
  - В режиме `live` запросы идут на `VITE_API_URL_LIVE`
- `VITE_API_URL_LIVE` - URL API для production режима (используется только если `VITE_APP_MODE=live`)
